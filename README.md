...
基于上传文档的本地智能问答系统。支持语义检索问答、表格精确统计、最新记录查询，Docker 一键部署，数据全部保存在本地。
...
│   └── api/
│       ├── chat/         # 问答接口（统计 / 最新记录 / RAG 三分支）
│       ├── upload/       # 文档上传与覆盖更新
│       ├── documents/    # 文档列表与删除
│       └── history/      # 聊天历史
├── components/           # 聊天、文档、历史、图表等组件
├── lib/                  # 核心逻辑
│   ├── rag.ts            # 检索、Prompt 组装、入库
